...
# Indicates whether processing should start or not
...
for dirpath, dirnames, filenames in os.walk(base_directory):
    for filename in filenames:
        if filename.endswith('.txt'):
            # Check if the current file or directory is or is past "LeBlanc"
            if not start_processing:
                if "LeBlanc" in dirpath or "LeBlanc" in filename:
                    start_processing = True
                else:
                    continue  # Skip to the next file or directory
            ...
            if formatted_text:
                ...
                print("\nFailed to format text or received")
